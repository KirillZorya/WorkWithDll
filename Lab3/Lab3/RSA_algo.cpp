...
unsigned __int64 Multip(unsigned __int64 a, unsigned __int64 b, unsigned __int64 m) {
    if (b == 1ull)
        return a;
    if (b % 2ull == 0) {
        unsigned __int64 t = Multip(a, b / 2ull, m);
        return (2ull * t) % m;
    }
    return (Multip(a, b - 1ull, m) + a) % m;
}
...
unsigned __int64 Pows(unsigned __int64 a, unsigned __int64 b, unsigned __int64 m) {
    ...
    if (b % 2ull == 0) {
        ...
        return Multip(t, t, m) % m;
    }
    return (Multip(Pows(a, b - 1ull, m), a, m)) % m;
}
